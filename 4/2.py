...
NN = f.NeuralNetwork(Xtrain,Ytrain) # инициализируем сетку на наших данных
N_epoch = 50
for i in range(N_epoch):
    
    print ("\n\nfor iteration # " + str(i) + "\n")
    print ("Loss: \n" + str(np.mean(np.square(Ytrain - NN.feedforward()))))
    pred = NN.feedforward_test(Xtest)
    #print(pred)
    pred = pd.DataFrame(pred, columns=['predictions'])
    pred = pred.round().to_numpy()
    tmp_y = np.array([1.0 if i == True else 0.0 for i in Ytest]).reshape(-1, 1)
    print("Accurace: \n"+str(accuracy_score(pred, tmp_y)))
    losses.append(np.mean(np.square(Ytrain - NN.feedforward())))
...
plt.xlabel("Эпоха")
plt.ylabel("Точность(синая линия) и среднекваратические потери(красная линия)")

                     
print("\n Итоговые веса на 1м слое = \n", NN.weights1)
print("\n Итоговые веса на 2м слое = \n", NN.weights2)
plt.show()
